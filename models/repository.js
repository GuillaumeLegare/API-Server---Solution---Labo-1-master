...
class Repository {
    ...
    getAll(params = null) {
        ...
        if (params) {
            ...
            for(let obj of objectsList){
                if(this.filterInfo(obj, searchKeys))
                    filteredAndSortedObjects.push(obj)
            }
            
            // todo sort
            for(let i=0;i<filteredAndSortedObjects.length;++i){
                for(let j = 0; j<filteredAndSortedObjects.length;++j){
                    let compare = this.innerCompare(filteredAndSortedObjects[i],filteredAndSortedObjects[j]);

                    if(sortKeys[0]){
                        switch(compare){
                            case 1:
                                let val = filteredAndSortedObjects[i];
                                filteredAndSortedObjects[i] = filteredAndSortedObjects[j];
                                filteredAndSortedObjects[i] = val;
                            break;
                        }
                    }else{
                        switch(compare){
    ...
    filterInfo(filterObj, arrSearchKey){
        var bool = true;
        for(let uniqueKey of arrSearchKey){
            if(!(this.valueMatch(filterObj[uniqueKey.key], uniqueKey.value)))
                bool = false;
        }
        return bool;
    }
